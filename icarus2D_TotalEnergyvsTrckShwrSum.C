...
//const Cut kITrueECut = kITrueE > 0.f; // selecting only energy values that are greater than 0
...
void icarus2D_TotalEnergyvsTrckShwrSum(const std::string inputName = "/pnfs/sbn/persistent/analysis/CAF/202106PAC/workshop_SBNWorkshop0421_prodoverlay_corsika_cosmics_cosmics_proton_genie_nu_spill_gsimple-config_caf_icarus/reduced_flatcafs/*.flat.caf.root")
{
  ...
  const Binning binsE = Binning::Simple(20, 0, 7);
  ...
  const HistAxis axTotalTrueE("ICARUS True Energy", binsE, kITrueE);

  //Spectrum(Spectrumloader,HistAxis,Cut)
  Spectrum sTrksandShws(loader, axTrksandShws, kNoCut);
  Spectrum sTotalTrueE(loader, axTotalTrueE, kNoCut);

  //Spectrum(Spectrumloader ,HistAxisX HistAxisY, SpillCut, Cut)
  Spectrum sTrkShwvsTrueE(loader, axTrksandShws, axTotalTrueE, kNoSpillCut, kNoCut);
  ...
  leg->AddEntry(hTotalTrueE, "ICARUS True Energy", "l");
  leg->Draw();
  c1->Print("sbnd_icarus_plots/icarusplots/icarusTrk&ShwSumvsTrueE.png");
  ...
  hTrkShwvsTrueE->SetYTitle("ICARUS True Energy [GeV]");
  hTrkShwvsTrueE->Draw("colz");
  c2->Print("sbnd_icarus_plots/icarusplots/icarusTrk&ShwSumvsTrueE_2D.png");
